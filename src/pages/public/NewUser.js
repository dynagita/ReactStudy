        ...
        .then(data => {
          showModal(false);
            if(data.success){
              ...
              showToast({type: 'error', show: true, timems: 5000, title: 'Error', message: data.error});
            }
        })
        .catch(res => {
          showModal(false);
          showToast({type: 'error', show: true, timems: 5000, title: 'Error', message: "An unexpected error has ocurred. Please try again."});
        });
      }
    }
    return (
